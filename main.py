...
def load_file():
    """加载数据"""
    # 加载数据集
    null = pd.read_csv(Config.new_20w_List[0], header=None, index_col=None)  # null
    like = pd.read_csv(Config.new_20w_List[1], header=None, index_col=None)  # like
    sad = pd.read_csv(Config.new_20w_List[2], header=None, index_col=None)  # sad
    disgust = pd.read_csv(Config.new_20w_List[3], header=None, index_col=None)  # disgust
    anger = pd.read_csv(Config.new_20w_List[4], header=None, index_col=None)  # anger
    happy = pd.read_csv(Config.new_20w_List[5], header=None, index_col=None)  # happy
...
def create_dictionaries(model=None, combined=None):
    ...
    if (combined is not None) and (model is not None):
        if os.path.exists(Config.combined_path) and os.path.exists(Config.w2vec_path) and os.path.exists(Config.w2indx_path):    # 已写入
            print("加载词典")
